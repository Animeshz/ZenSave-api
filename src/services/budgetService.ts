...
const getAllCategoriesForUser = async (
    ...
    dbClient.$queryRaw`SELECT users_user_id,
                       ...
                       WHERE users_user_id = ${userId}
                         AND status = ${ZENSAVE.CATEGORY_STATUS.ACTIVE} `;
...
const calculateBudgetBalance = async (
    ...
    const categories = await getAllCategoriesForUser(userId, budgetId as number, prismaTx);
...
const getSumAmountsForBudget = async (userId, budget, dbClient = undefined) => performDatabaseRequest(async (prismaTx) => {
    const budgetId = budget.budget_id;
    const month = parseInt(budget.month, 10);
    const year = parseInt(budget.year, 10);
    const isOpen = budget.is_open;

    const categories: any = await getAllCategoriesForUser(userId, budgetId, prismaTx);
    let balanceCredit = 0;
    let balanceDebit = 0;
...
const getExpandedBudgetAmountsData = async (userId: bigint, budget, dbClient = prisma) => {
        const promises = [];
        promises.push(calculateBudgetBalance(userId, budget, dbClient))
        promises.push(calculateBudgetBalanceChangePercentage(
            userId,
            budget,
            budget.balance_value,
            dbClient
        ))
        promises.push(getSumAmountsForBudget(userId, budget, dbClient))
...
const getAllBudgetsForUser = async (userId: bigint, status: string, dbClient = prisma) => {
    ...
    }> = await dbClient.budgets.findMany({
        where: whereCondition,
        orderBy: [{year: 'desc'}, {month: 'desc'}],
    })
    ...
    for await (const budget of budgetsList) {
        budgetPromises.push(getExpandedBudgetAmountsData(userId, budget, dbClient))
    }
    ...
    const result: any = await dbClient.$queryRaw`SELECT sum(amount) as 'amount'
                                               ...
                                               where users_user_id = ${userId}
                                                 ...
                                                 and (transactions.type = ${
                                                   ZENSAVE.TRX_TYPES.EXPENSE
                                                 } or transactions.type = ${ZENSAVE.TRX_TYPES.TRANSFER})`;
