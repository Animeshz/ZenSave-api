...
const accountService = {
    ...
    getAccountsForUserWithAmounts: async (userId: bigint, onlyActive = false) => {
        const onlyActiveExcerpt = onlyActive ? `AND a.status = ${ZENSAVE.ACCOUNT_STATUS.ACTIVE}` : '';
        ...
        return prisma.$queryRaw`SELECT a.account_id,
                            ...
                            ORDER BY abs(balance) DESC, case when a.status = ${ZENSAVE.TRX_TYPES.EXPENSE} then 1 else 0 end`;
    },
    ...
    recalculateBalanceForAccountIncrementally: async (
        ...
        for (const trx of trxList) {
            ...
            if (
                trxType === ZENSAVE.TRX_TYPES.EXPENSE ||
                (trxType === ZENSAVE.TRX_TYPES.TRANSFER &&
    ...
    getBalancesSnapshotForMonthForUser: async (
        ...
        for(const account of accounts){
            if (includeInvestmentAccounts || account.type !== ZENSAVE.ACCOUNT_TYPES.INVESTING) {
    ...
    getAllAccountsForUserWithAmounts: async (
        ...
        performDatabaseRequest(async (dbTx) => {
            return dbTx.$queryRaw`SELECT a.account_id,
                              ...
                              FROM accounts a
                              WHERE users_user_id = ${userId}
                                AND a.status LIKE ${onlyActive ? ZENSAVE.ACCOUNT_STATUS.ACTIVE : '%'}
                              ORDER BY abs(balance) DESC, case when a.status = ${
                                ZENSAVE.ACCOUNT_STATUS.INACTIVE
                              } then 1 else 0 end`;
        }, dbClient),
