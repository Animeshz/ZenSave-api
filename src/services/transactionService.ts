  ...
  await performDatabaseRequest(async (prismaTx) => {
    ...
    switch (trx.type) {
      case ZENSAVE.TRX_TYPES.INCOME:
        ...
        break;
      case ZENSAVE.TRX_TYPES.EXPENSE:
        newBalance = await AccountService.recalculateBalanceForAccountIncrementally(
          trx.account_from_id,
          trx.date_timestamp - 1,
          DateTimeUtils.getCurrentUnixTimestamp() + 1,
        ...
        break;
      case ZENSAVE.TRX_TYPES.TRANSFER:
      default:
        newBalance = await AccountService.recalculateBalanceForAccountIncrementally(
          trx.account_to_id,
          trx.date_timestamp - 1,
...
const autoCategorizeTransaction = async (
  ...
  performDatabaseRequest(async (prismaTx) => {
    ...
    return {
      ...
      selectedAccountFromID: matchedRule?.assign_account_from_id ?? accountsFromId, //(type == ZENSAVE.TRX_TYPES.INCOME) ? matchedRule?.assign_account_from_id : accountsFromId,
      selectedAccountToID: matchedRule?.assign_account_to_id ?? accountsToId, //(type == ZENSAVE.TRX_TYPES.INCOME) ? accountsToId :  matchedRule?.assign_account_to_id,//matchedRule?.assign_account_to_id || accountsToId,
  ...
  for (const trx of trxList) {
    promises.push(
      autoCategorizeTransaction(
        ...
        trx.amount,
        trx.type,
        trx.type == ZENSAVE.TRX_TYPES.INCOME ? null : accountId,
        trx.type != ZENSAVE.TRX_TYPES.INCOME ? null : accountId,
        trx.date,
        dbClient
      )
